...
function render(data){ $('updatedAt').textContent='同步时间：'+new Date(data.publishedAt).toLocaleString('zh-CN'); $('groups').innerHTML=(data.groups||[]).map(group=>`<section class="group"><h3>${escapeHtml(group.name)}</h3>${group.students.map(renderStudent).join('')}</section>`).join('')||'<section class="panel unlock">暂无学生任务数据。</section>'; $('unlock').hidden=true; $('board').hidden=false; }
