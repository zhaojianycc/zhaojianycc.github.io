...
function render(data){ $('updatedAt').textContent='更新时间：'+new Date(data.publishedAt).toLocaleString('zh-CN'); $('groups').innerHTML=(data.groups||[]).map(group=>`<section class="group"><h3>${escapeHtml(group.name)}</h3>${group.students.map(student=>`<article class="task"><strong>${escapeHtml(student.displayName)}</strong><div class="text">${escapeHtml(student.latestTask)}</div><time>${escapeHtml(student.meetingDate||'')}</time></article>`).join('')}</section>`).join('')||'<section class="panel unlock">暂无已发布任务。</section>'; const rows=data.overview||[]; $('overviewRows').innerHTML=rows.map(item=>`<tr><td>${escapeHtml(item.name)}</td><td>${escapeHtml(item.service||'未设置')}</td><td>${escapeHtml(item.projects||'未参与项目')}</td></tr>`).join(''); $('overviewPanel').hidden=!rows.length; $('unlock').hidden=true; $('board').hidden=false; }
