    ...
    for i, entry in enumerate(entries):
        ...
        if highlightAuthors: # highlight some authors 
            for highlightAuthor in highlightAuthors:
                p = re.compile(highlightAuthor+' ');
                author = p.sub("{{<font color= \#a8d6ff ><b>"+highlightAuthor+"</b></font>}} ", author);
        ...
        paper = entry['paperweb'] if 'paperweb' in entry else "";
        preprint = entry['preprintweb'] if 'preprintweb' in entry else "";
        slide = entry['slide'] if 'slide' in entry else "";
        if publishlink: # create link if publishlink is set 
            title = "[" + publishlink + " " + title +"]"
        addressAndDate = getAddressAndDate(entry);

        if volume and number and pages and paper and annotate:
            print """- \[%s%d\] %s, "%s", %s, vol. %s, no. %s, pp. %s, %s. {{<a href=\"%s\">(paper) </a><font color=red><b>(%s)</b></font>}} """ % \
                (prefix, count, author, title, booktitle, volume, number, pages, addressAndDate, paper, annotate);
        elif paper and annotate:
            print """- \[%s%d\] %s, "%s", %s, %s. {{<a href=\"%s\">(paper) </a><font color=red><b>(%s)</b></font>}} """ % \
                (prefix, count, author, title, booktitle, addressAndDate, paper, annotate); 
        elif preprint and annotate:
            print """- \[%s%d\] %s, "%s", %s, %s. {{<a href=\"%s\">(preprint) </a><font color=red><b>(%s)</b></font>}} """ % \
                (prefix, count, author, title, booktitle, addressAndDate, preprint, annotate); 
        elif annotate:
            print """- \[%s%d\] %s, "%s", %s, %s. {{<font color=red><b>(%s)</b></font>}} """ % \
                (prefix, count, author, title, booktitle, addressAndDate, annotate);
        elif preprint:
            print """- \[%s%d\] %s, "%s", %s, %s. {{<a href=\'%s\'>(preprint)</a>}}""" % \
                (prefix, count, author, title, booktitle, addressAndDate, preprint);
        elif volume and number and pages and paper:
            print """- \[%s%d\] %s, "%s", %s, vol. %s, no. %s, pp. %s, %s. {{<a href=\'%s\'>(paper)</a>}}""" % \
                (prefix, count, author, title, booktitle, volume, number, pages, addressAndDate, paper);
        elif pages and paper:
            print """- \[%s%d\] %s, "%s", %s, pp. %s, %s. {{<a href=\'%s\'>(paper)</a>}}""" % \
                (prefix, count, author, title, booktitle, pages, addressAndDate, paper);
        elif paper:
            print """- \[%s%d\] %s, "%s", %s, %s. {{<a href=\'%s\'>(paper)</a>}}""" % \
                (prefix, count, author, title, booktitle, addressAndDate, paper);
        elif pages:
            print """- \[%s%d\] %s, "%s", %s, pp. %s, %s. """ % \
                (prefix, count, author, title, booktitle, pages, addressAndDate);
        else:
            print """- \[%s%d\] %s, "%s", %s, %s. """ % \
...
%{{{
    ...
    for i, entry in enumerate(entries):
        ...
        if highlightAuthors: # highlight some authors 
            for highlightAuthor in highlightAuthors:
                p = re.compile(highlightAuthor+' ');
                author = p.sub("\\\\textbf{"+highlightAuthor+"} ", author);
                #author = p.sub("\\\\textcolor[rgb]{0.00,0.07,1.00}{\\\\textbf{"+highlightAuthor+"}} ", author);
